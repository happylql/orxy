<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column :label="$t('information.playerId')" prop="id" align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('information.nickname')" width="200px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.nickname }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('information.serverName')" width="200px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.serverName }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('information.channelName')" width="200px" align="center">
        <template slot-scope="scope">
...
<script>
import { fetchPlayerList, sealAccount, forbidChat } from "~/api/information";
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$axios(fetchPlayerList(this.listQuery)).then(response => {
    ...
    handleSealAccount(row, status) {
      this.$axios(sealAccount(row.id)).then(response => {
        this.$message({
          message: "操作成功",
          type: "success"
        });
    ...
    handleForbidChat(row, status) {
      this.$axios(forbidChat(row.id)).then(response => {
        this.$message({
          message: "操作成功",
          type: "success"
        });
        row.chatStatus = status;
      });
    },
    handleSendEmail(row) {
